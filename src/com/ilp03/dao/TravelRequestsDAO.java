...
public class TravelRequestsDAO {
	...
	/*public static ArrayList<TravelRequests> getTravelExpenseDataBasedOnEmployees(Connection connection, String employeeName) {

		ArrayList<TravelRequests> travelRequestsList = new ArrayList<TravelRequests>();
		try {
			...
			while (resultSet.next()) {
				...
				travelRequestsList.add(travelRequests);
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public static void insertIntoTravelRequests(Connection connection,int employeeID,Date startDate,Date endDate,String destination,String travelPurpose,String transportType) {
		try {
			PreparedStatement preparedStatement = connection.prepareStatement(
					" INSERT INTO travel_requests (employee_id, start_date, end_date, destination, "
					+ "travel_purpose,transportation_type,status)VALUES (?,?,?,?,?,?,'pending')");
 
			preparedStatement.setInt(1,employeeID );
			preparedStatement.setDate(2,startDate );
			preparedStatement.setDate(3,endDate );
			preparedStatement.setString(4,destination );
			preparedStatement.setString(5,travelPurpose );
			preparedStatement.setString(6,transportType );
